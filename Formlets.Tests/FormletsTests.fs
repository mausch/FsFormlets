        ...
        testList "Applicatives" [
            testCase "NameGen" <| fun _ ->
                let t = 
                    ...
                    |> NameGen.ap NameGen.nextName
                    |> NameGen.run
                assertEqual "two name generations" ("f0", "f1") t
            ...
            testCase "XmlWriter" <| fun _ ->
                ...
                let nodes, v = 
                    ...
                    |> a.ap ([XText "b"],3)
                assertEqual "sum" 5 v
                assertEqual "list length" 2 nodes.Length
                match nodes with
                | [TextV "a"; TextV "b"] -> ()
            ...
            testCase "ErrorList" <| fun _ ->
                ...
                let errors, v = 
                    ...
                    |> a.ap (["c"; "d"], 3)
                assertEqual "sum" 5 v
                assertEqual "list length" 4 errors.Length
                assertEqual "errors" ["a";"b";"c";"d"] errors
            ...
            testCase "XmlWriter composed with ErrorList" <| fun _ ->
                ...
                let nodes, (errors, v) =
                    ...
                    |> ap ([XText "b"], (["c";"d"],3))
                assertEqual "sum" 5 v
                assertEqual "errors length" 4 errors.Length
                assertEqual "nodes length" 2 nodes.Length
                assertEqual "errors" ["a";"b";"c";"d"] errors
                match nodes with
                | [TextV "a"; TextV "b"] -> ()
                | _ -> failtestf "Unexpected %A" nodes
